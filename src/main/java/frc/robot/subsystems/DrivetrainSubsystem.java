...
public class DrivetrainSubsystem implements Subsystem {
    ...
    public void drive(double xSpeed, double ySpeed, double angularVelocity, boolean fieldRelative,
        ...
        SwerveModuleState[] moduleStates = kinematics.toSwerveModuleStates(
                fieldRelative ? ChassisSpeeds.fromFieldRelativeSpeeds(chassisSpeeds, getAdjustedRotation())
                        : chassisSpeeds);
    ...
    public boolean checkMotion() {
        if(Math.sqrt(currentChassisSpeeds.vxMetersPerSecond*currentChassisSpeeds.vxMetersPerSecond + currentChassisSpeeds.vyMetersPerSecond*currentChassisSpeeds.vyMetersPerSecond) < 0.05 && currentChassisSpeeds.omegaRadiansPerSecond < 0.02)
        {
            frontLeft.correctRelativeEncoder();
            frontRight.correctRelativeEncoder();
            backLeft.correctRelativeEncoder();
            backRight.correctRelativeEncoder();
            return true;
        }
        return false;
    }
    ...
    public Command joystickDriveCommand(DoubleSupplier xAxis, DoubleSupplier yAxis, DoubleSupplier rotAxis) {
        return Commands.run(() -> {
            ...
            ChassisSpeeds speeds = joystickAxesToChassisSpeeds(xAxis.getAsDouble(), yAxis.getAsDouble(),
                    rotAxis.getAsDouble());
    ...
    public Command driveAndPointToSpeakerCommand(DoubleSupplier xAxis, DoubleSupplier yAxis) {
        return pointToSpeakerWithSpeedsCommand(
                () -> (joystickAxesToChassisSpeeds(xAxis.getAsDouble(), yAxis.getAsDouble(), 0)));
    }
    ...
    public Command pointToSpeakerWithSpeedsCommand(Supplier<ChassisSpeeds> speedsSupplier) {
        return Commands.run(() -> {
            ...
            // assuming Blue Speaker, and Blue is Left/Red is Right and speakers/amps are on
            // the top half of the arena
            double desiredAngle = Math.atan(yDistance / xDistance);

            drive(speedsSupplier.get().vxMetersPerSecond, speedsSupplier.get().vyMetersPerSecond,
                    rotationController.calculate(getPose().getRotation().getRadians(), desiredAngle), true, true);
